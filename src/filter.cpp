...
void C3DeflattenFilter::LoadIni() {
    ...
    m_cfg.cropLeft = m_cfg.cropTop = m_cfg.cropRight = m_cfg.cropBottom = 0;
    ...
    for (;;) {
        ...
        if (cfg.autoCropBlackBars) {
            if (--m_cropDetectCounter <= 0) {
                ...
                m_cfg.cropLeft=m_cropLeft; m_cfg.cropTop=m_cropTop;
        ...
        if (SUCCEEDED(hr) && (cW<w || cH<h)) {
            ...
            result.data=std::move(full);result.width=w;result.height=h;
        }
        if (SUCCEEDED(hr)) {
