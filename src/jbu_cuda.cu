...
// Reference: Kopf et al. 2007; github.com/morsingher/jbu_cuda
#include <cuda_runtime.h>
#include "jbu_cuda.h"

// Pass 1: sample guide BGRA bilinearly at low-res grid centres → float luma [0,1]
    ...
    float* __restrict__ glr, int lrW, int lrH)
{
    ...
    float fx=(kx+.5f)*hrW/(float)lrW-.5f, fy=(ky+.5f)*hrH/(float)lrH-.5f;
    int x0=max(0,(int)fx),x1=min(hrW-1,x0+1),y0=max(0,(int)fy),y1=min(hrH-1,y0+1);
...
// Pass 2: JBU upsample — one thread per output pixel
...
int jbu_cuda(const float* dlr, int lrW, int lrH,
             ...
             float* dhr, float ss, float sc, int radius, void* stream)
{
    cudaStream_t st=(cudaStream_t)stream;
    float* glr=nullptr;
    if(cudaMalloc(&glr,(size_t)lrW*lrH*sizeof(float))!=cudaSuccess) return 1;
    dim3 b(16,16);
